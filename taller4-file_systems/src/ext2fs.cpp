...
{
	//TODO: Ejercicio 2
	unsigned int grupo = blockgroup_for_inode(inode_number);
	Ext2FSBlockGroupDescriptor* descriptor = block_group(grupo);
	unsigned int tabla_inodos = descriptor->inode_table;
	unsigned int tamano_inodo = sizeof(Ext2FSInode);
	unsigned char* buff = (unsigned char*) malloc(1024);
	read_block(tabla_inodos + blockgroup_inode_index(inode_number) * tamano_inodo, buff);
	unsigned char* inodo = (unsigned char*) malloc(tamano_inodo);
	for (unsigned int i = 0; i < tamano_inodo; i++) inodo[i] = buff[i];
	return (Ext2FSInode*) inodo;
}

unsigned int Ext2FS::get_block_address(struct Ext2FSInode * inode, unsigned int block_number)
{
	// tenemos 1024 B x bloque / 4 B x dirección = 256 direcciones en un bloque-tabla
	//TODO: Ejercicio 1
	// read_block(block_number, )
	if (block_number < 12){
		return inode->block[block_number];
	} else if (block_number <= 256+12){ 	//indireccion simple
		unsigned char* buff = (unsigned char*) malloc(1024);
		read_block(inode->block[13], buff);
		return (unsigned int) &buff[block_number-12];
	} else if (block_number <= 256*256+12){ //indireccion doble
		unsigned char* buff = (unsigned char*) malloc(1024);
		read_block(inode->block[14], buff);
		read_block(buff[block_number/256-12], buff);
		return (unsigned int) &buff[block_number-12];
	} else { 								//indireccion triple
		unsigned char* buff = (unsigned char*) malloc(1024);
		read_block(inode->block[15], buff);
		read_block(buff[block_number/256-12], buff);
		read_block(buff[block_number/(256*256)-12], buff);
		return (unsigned int) &buff[block_number-12];
	}
}
...
{
	unsigned int block_size = 1024 << _superblock->log_block_size;
	unsigned int sectors_per_block = block_size / SECTOR_SIZE;			// 512?????????
...
struct Ext2FSInode * Ext2FS::get_file_inode_from_dir_inode(struct Ext2FSInode * from, const char * filename)
{
	...
	Ext2FSDirEntry buff[cont];
	for (unsigned int i = 0; i < 12+256+256^2+256^3-1; i++){
		unsigned int bloque1 = get_block_address(from, i);
		unsigned int bloque2 = get_block_address(from, i+1);
		unsigned int cont;
		read_block(bloque1, buff);
		
		// comparar con filename con string compare
		// si llega a cincidir return funcion2(buff.inodo)
		cont = buff->record_length;
		buff[comt]
