...
struct Ext2FSInode * Ext2FS::load_inode(unsigned int inode_number)
{
	...
	Ext2FSBlockGroupDescriptor* gds = block_group(group_number);
	unsigned int inodes_per_block = block_size / sizeof(Ext2FSInode);
	unsigned int table_block = inode_index / inodes_per_block;
	unsigned int table_index = inode_index % inodes_per_block;
	unsigned int addr = gds->inode_table + table_block;
	
	unsigned char* buffer = (unsigned char*)malloc(block_size);
	read_block(addr, buffer);

	Ext2FSInode* inode = (Ext2FSInode*)malloc(sizeof(Ext2FSInode));
	memcpy(inode, buffer + table_index * sizeof(Ext2FSInode), sizeof(Ext2FSInode));
...
unsigned int Ext2FS::get_block_address(struct Ext2FSInode * inode, unsigned int block_number)
{
	...
	unsigned int addr;
	unsigned int num_indireccion;
	
	if (block_number < 1024) {
		addr = inode->block[12];
		num_indireccion = 0;
	}
	else if (block_number - 1024 < 1024 * 1024) {
		addr = inode->block[13];
		num_indireccion = 1;
	}
	else {
	...
	read_block(addr, (unsigned char*)buffer);

	while(num_indireccion > 0) {
		unsigned int entry_skip = pow(1024, num_indireccion);
		unsigned int index = block_number / entry_skip;

		read_block(buffer[index], (unsigned char*)buffer);
		
		block_number -= entry_skip * index;
		num_indireccion--;
	}
...
struct Ext2FSInode * Ext2FS::get_file_inode_from_dir_inode(struct Ext2FSInode * from, const char * filename)
{
	...
	unsigned int block_size = 1024 >> superblock()->log_block_size;
	unsigned char* buffer = (unsigned char*)malloc(2 * block_size);

	for (int block_number = 0; block_number < from->blocks; block_number++) {
		read_block(get_block_address(from, block_number + 0), buffer + 0);
		read_block(get_block_address(from, block_number + 1), buffer + block_size);

		Ext2FSDirEntry* entry = (Ext2FSDirEntry*)buffer;
		do {
			// std::cout << (int)entry->name_length << "--" << entry->name << " INODE=" << entry->inode << std::endl;
			if (strncmp(entry->name, filename, std::min((int)entry->name_length, len)) == 0) {
				return load_inode(entry->inode);
			}

			buffer += entry->record_length;
			entry = (Ext2FSDirEntry*)buffer;
		} while(entry->record_length > 0);
	}
